...
func genCLICobra(doc *spec.Document, opts *genCLIOptions) (map[string][]byte, error) {
	...
	// Render run.go.
	runContent, err := renderCobraTemplate("templates/code/cobra/gencobra/run.tmpl", funcMap, allCmdsData)
	if err != nil {
	...
	for _, cmdFile := range cmdFiles {
		tmplPath := "templates/code/cobra/gencobra/command.tmpl"
