...
func loadConfig() {
	...
	// Try YAML config
	if data, err := os.ReadFile(expandTilde("~/.ocli/config.yaml")); err == nil {
...
file) in the order they are declared, falling back to the zero value when no source yields
a usable result. The merged pflag.FlagSet passed in already contains both local and
inherited persistent flags at RunE time.
*/
...
// alternative sources, matching urfave/cli behavior for cross-framework parity.
...
// one value coercible to T; otherwise an empty slice. get is the bound pflag getter for the
// concrete element type (e.g. fs.GetStringArray).
